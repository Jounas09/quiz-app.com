    ...
    <div class="card">
        <div class="card-body">
            ...
            <form action="{{ route('test.update',['test'=>$test->id]) }}" method="POST">
                @csrf
                @method('PUT')
                <input type="hidden" id="FormControlInputBank" value="{{ $test->id }}" name="test" />
                ...
                <h2>{{ English::Total_questions_text }} {{ count(json_decode($banks->questions_json)) }}</h2>
                    ...
                    <input value="{{ $test->question_number }}" type="number" class="form-control" id="FormControlInputNumberQuestions" name="questions" placeholder="{{ English::Number_questions }}" required>
